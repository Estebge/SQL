...
Pratique pour essayer le projet sans toucher à son poste : le serveur tourne dans un
conteneur, et tout est supprimé en une commande à la fin.
...
# 2. Démarrer un serveur MySQL (la base qualite_air est créée automatiquement)
docker run --name qualite-air \
  -e MYSQL_ROOT_PASSWORD=root \
  -e MYSQL_DATABASE=qualite_air \
  -p 3306:3306 -d mysql:8.0

# 3. Attendre que le serveur soit prêt (quelques secondes au premier démarrage)
